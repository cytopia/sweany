...
class ForumForumsTable extends Table
{
	...
	public $hasMany = array(
		'LastThread'	=> array(
			...
			'subQueries'	=> array(
				'username'		=> 'SELECT username		FROM core_users	 WHERE core_users.id  = LastThread.fk_user_id',
				...
				'post_username'	=> 'SELECT username		FROM core_users	 WHERE core_users.id  = (SELECT fk_user_id FROM forum_posts WHERE forum_posts.id = LastThread.last_post_id)',
			),
		...
		'Thread'	=> array(
			...
			'subQueries'	=> array(
				'username'		=> 'SELECT username		FROM core_users	 WHERE core_users.id = Thread.fk_user_id',
