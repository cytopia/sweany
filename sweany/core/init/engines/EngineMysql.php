...
class EngineMysql extends \Core\Init\CoreAbstract
{
	...
	**********************************************************************************************************************************/
	...
	public static function selectNumRows($query)
	{
		...
		if (!$result)
		{
			\SysLog::sqlError('numRows', 'mysql_query failed', $query, array(mysql_errno(self::$link),mysql_error(self::$link), $time));
			return (-1);
		}
		$start	= microtime(true);
		$num	= mysql_numrows($result);
		$time	= (microtime(true) - $start)+$time;	// measure again for 2nd query

		if (!mysql_free_result($result))
			\SysLog::sqlError('Cannot Free Result', 'mysql_free_result failed', $query, array(mysql_errno(self::$link),mysql_error(self::$link), $time));

		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('numRows', null, $query, $num, $time);
	...
	**********************************************************************************************************************************/
	...
	public static function fetchRowById($table, $id, $fields = array())
	{
		...
		if ( !isset($data[0]) )
		{
			\SysLog::sqlWarn('fetchRowById', 'returning empty array', self::$query, $data);
			return array();
		}
		if ( count($data) > 1 )
		{
	...
	public static function incrementField($table, $field, $where, $get_update_id = null, $other_fields = array())
	{
		...
		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('incrementField', null, $query, null, $time);
	...
	**********************************************************************************************************************************/
	...
	public static function getEnumValues($table, $field)
	{
		...
		$data = self::select($query);
		$data = $data[0]['COLUMN_TYPE'];
		$data = explode('\',\'', substr($data,6,-2));	// enum('first','bla',...,'last')
		return $data;
	}
	...
	private static function _setServerFlags($flags = array())
	{
		...
		if (!$result)
		{
			\SysLog::sqlError('set Server flags', 'mysql_query failed', $query, array(mysql_errno(self::$link),mysql_error(self::$link), $time));
			return (0);
		}

		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('SET flags', null, $query, null, $time);
