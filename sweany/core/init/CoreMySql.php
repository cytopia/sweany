...
class CoreMySql extends CoreAbstract
{
	...
	**********************************************************************************************************************************/
	...
	public static function select($query)
	{
		...
		if (!$result)
		{
			\SysLog::sqlError('select', self::_formatError($query, $time));
			return (-1);
		}
		...
		if (!mysql_free_result($result))
			\SysLog::sqlError('Cannot Free Result', self::_formatError($query, $time), $data);

		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('select', self::_formatError($query, $time), $data);
	...
	public static function selectNumRows($query)
	{
		...
		if (!$result)
		{
			\SysLog::sqlError('numRows', self::_formatError($query, $time));
			return (-1);
		}
		...
		if (!mysql_free_result($result))
			\SysLog::sqlError('Cannot Free Result', self::_formatError($query, $time));

		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('numRows', self::_formatError($query, $time));
	...
	**********************************************************************************************************************************/
	...
	public static function fetchField($table, $field, $where)
	{
		...
		{
			\SysLog::sqlWarn('fetchField', 'returning empty field: '.self::$query, null);
			return null;
		}
		if ( count($data) > 1 )
		{
			\SysLog::sqlWarn('fetchField', 'result hash more than one row: '.self::$query, null, $data);
		}
		return $data[0][$field];
	}
	...
	public static function fetchRowById($table, $id, $fields = array())
	{
		...
		if ( !isset($data[0]) )
		{
			\SysLog::sqlWarn('fetchRowById', 'returning empty array: '.self::$query);
			return array();
		}
		if ( count($data) > 1 )
		{
			\SysLog::sqlWarn('fetchRowById', 'result hash more than one row: '.self::$query, $data);
		}
	...
	public static function insertRow($table, $field_array = array())
	{
		...
		if (!$result)
		{
			\SysLog::sqlError('insertRow', self::_formatError($query, $time));
			return (-1);
		}
		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('insertRow', self::_formatError($query, $time), NULL);
	...
	public static function update($table, $field_array = array(), $where)
	{
		...
		if (!$result)
		{
			\SysLog::sqlError('updateRow', self::_formatError($query, $time));
			return (-1);
		}

		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('updateRow', self::_formatError($query, $time));
	...
	public static function delete($table, $where)
	{
		...
		if (!$result)
		{
			\SysLog::sqlError('delete', self::_formatError($query, $time));
			return (-1);
		}

		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('delete', self::_formatError($query, $time), null);
	...
	private static function _setServerFlags($flags = array())
	{
		...
		if (!$result)
		{
			\SysLog::sqlError('set Server flags', self::_formatError($query, $time));
			return (0);
		}

		\SysLog::sqlAppendTime($time);
		\SysLog::sqlInfo('SET flags', self::_formatError($query, $time));
	...
	{
		return (count($fields)) ? implode(',', array_map( create_function('$key, $val', 'return " ".$key." AS ".$val;'), array_values($fields), array_keys($fields))) : '*';
	}
	...
	{
		return (count($order)>0) ? 'ORDER BY '.implode(', ', array_map( create_function('$key, $val', 'return "$key ".$val;'), array_keys($order), array_values($order))) : '';
	}
