...
class Callback extends CoreTemplate
{
	...
	public static function initialize()
	{
		...
		if ($is_route)
		{
			$blockPath	= Url::getBlockPath();
			$blockCtrl	= Url::getBlockController();
			$blockMthd	= Url::getBlockMethod();
			$blockParams= Url::getBlockParams();
			
			if ( file_exists($blockPath.DS.$blockCtrl.'Block.php') && self::_blockMethodIsCallable($blockCtrl, $blockMthd) )
			{
				Log::setWarn('Callback', '[AJAX-Like Request]: '.$blockCtrl.'->'.$blockMthd.'('.implode(',', $blockParams).') in: '.$blockPath);

				require($blockPath.DS.$blockCtrl.'Block.php');
				require($blockPath.DS.$blockCtrl.'BlockModel.php');

				self::$object = array(
					'class'		=> $blockCtrl.'Block',
					'method'	=> $blockMthd,
					'params'	=> $blockParams,
				);
				return true;
			...
			{
				Log::setError('ERRR', $blockPath.DS.$blockCtrl.'Block.php');
				return false;
			}
