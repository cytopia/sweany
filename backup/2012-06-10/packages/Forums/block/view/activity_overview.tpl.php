	...
	<?php foreach ($forumThreads as $thread):?>
		<tr>
			<td>
				...
				<span style="font-size:10px; line-height:120%;"><?php echo Link::profilesView($thread['username'], array($thread['fk_user_id'])); ?></span>
			</td>
			<td style="width:100px;">
				...
				<?php if ($thread['last_post_id']): ?>
					...
					von <?php echo Link::profilesView($thread['last_post_username'], array($thread['last_post_user_id'])); ?>
				<?php else: ?>
					<?php echo getFormattedDate($thread['created']); ?> <span style="color:gray"><?php echo getFormattedTime($thread['created']);?></span><br/>
					von <?php echo Link::profilesView($thread['username'], array($thread['fk_user_id'])); ?>
				<?php endif; ?>
				</div>
			</td>
