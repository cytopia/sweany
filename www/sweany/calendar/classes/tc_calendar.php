...
class tc_calendar{
	...
	function getDayHeaders(){
		$rtn_hdrs = array();
		$hdrs = array("0"=>"So", "1"=>"Mo", "2"=>"Di", "3"=>"Mi", "4"=>"Do", "5"=>"Fr", "6"=>"Sa");
	...
	function returnScript()
	{
		...
		$myString .= $this->returnHidden();

		//check whether it is a date picker
		if($this->date_picker)
		{
			$myString .= ("<div style=\"position: relative; z-index: $this->zindex; float: left;\" id=\"container_".$this->objname."\" onmouseover=\"javascript:focusCalendar('$this->objname');\" onmouseout=\"javascript:unFocusCalendar('$this->objname', $this->zindex);\">");

			if($this->show_input)
			{
				$myString .= $this->returnWriteDay();
				$myString .= $this->returnWriteMonth();
				$myString .= $this->returnWriteYear();
			}
			else
			{
				$myString .= (" <a href=\"javascript:toggleCalendar('".$this->objname."', $this->auto_hide, $this->auto_hide_time);\">");
				$myString .= $this->returnWriteDateContainer();
				$myString .= ("</a>");
			}

			$myString .= (" <a href=\"javascript:toggleCalendar('".$this->objname."', $this->auto_hide, $this->auto_hide_time);\">");
			$myString .= ("<img src=\"".$this->icon."\" id=\"tcbtn_".$this->objname."\" name=\"tcbtn_".$this->objname."\" border=\"0\" align=\"absmiddle\" />");
			$myString .= ("</a>");

			$myString .= $this->returnWriteCalendarContainer();
	...
	function returnWriteCalendarContainer()
	{
		$myString = '';
		$params = array();
		$params[] = "objname=".$this->objname;
		...
		//write the calendar container
		$myString .= ("<div id=\"div_".$this->objname."\" style=\"position:".$div_position.";visibility:".$div_display.";z-index:100;".$div_align."\" class=\"div_calendar calendar-border\" $mout_str $mover_str>");
		$myString .= ("<IFRAME id=\"".$this->objname."_frame\" src=\"".$this->path."calendar_form.php".$paramStr."\" frameBorder=\"0\" scrolling=\"no\" allowtransparency=\"true\" width=\"100%\" height=\"100%\" style=\"z-index: 100;\"></IFRAME>");
		$myString .= ("</div>");
		return $myString;
	}
	...
	//write the select box of days
	function returnWriteDay()
	{
		$myString	= '';
		$total_days = $this->total_days($this->month, $this->year);

		$myString .= ("<select name=\"".$this->objname."_day\" id=\"".$this->objname."_day\" onChange=\"javascript:tc_setDay('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcday\"".($this->rtl ? " dir=\"rtl\"" : "").">");
		$myString .= ("<option value=\"00\"".($this->rtl ? " dir=\"rtl\"" : "").">Day</option>");
		for($i=1; $i<=$total_days; $i++){
			$selected = ((int)$this->day == $i) ? " selected" : "";
			$myString .= ("<option value=\"".str_pad($i, 2 , "0", STR_PAD_LEFT)."\"$selected".($this->rtl ? " dir=\"rtl\"" : "").">$i</option>");
		}
		$myString .= ("</select> ");
		return $myString;
	}
	...
	//write the select box of months
	function returnWriteMonth()
	{
		$myString = '';
		$myString .= ("<select name=\"".$this->objname."_month\" id=\"".$this->objname."_month\" onChange=\"javascript:tc_setMonth('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcmonth\"".($this->rtl ? " dir=\"rtl\"" : "").">");
		$myString .= ("<option value=\"00\"".($this->rtl ? " dir=\"rtl\"" : "").">Month</option>");
	...
	//write the year textbox
	function returnWriteYear()
	{
		$myString = '';
		//echo("<input type=\"textbox\" name=\"".$this->objname."_year\" id=\"".$this->objname."_year\" value=\"$this->year\" maxlength=4 size=5 onBlur=\"javascript:tc_setYear('".$this->objname."', this.value, '$this->path');\" onKeyPress=\"javascript:if(yearEnter(event)){ tc_setYear('".$this->objname."', this.value, '$this->path'); return false; }\"> ");
		$myString .= ("<select name=\"".$this->objname."_year\" id=\"".$this->objname."_year\" onChange=\"javascript:tc_setYear('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcyear\"".($this->rtl ? " dir=\"rtl\"" : "").">");
		$myString .= ("<option value=\"0000\"".($this->rtl ? " dir=\"rtl\"" : "").">Year</option>");
		...
		for($i=$year_end; $i>=$year_start; $i--){
			$selected = ((int)$this->year == $i) ? " selected" : "";
			$myString .= ("<option value=\"$i\"$selected".($this->rtl ? " dir=\"rtl\"" : "").">$i</option>");
		}
		$myString .= ("</select> ");
		return $myString;
	}
	...
	{
		$myString = '';
		$myString .= $this->returneHidden('', $this->getDate());
		$myString .= $this->returneHidden('dp', $this->date_picker);
		$myString .= $this->returneHidden('year_start', $this->year_start);
		$myString .= $this->returneHidden('year_end', $this->year_end);

		//deprecated since version 3.61
		//$this->eHidden('mon', $this->startMonday);

		$myString .= $this->returneHidden('da1', $this->time_allow1);
		$myString .= $this->returneHidden('da2', $this->time_allow2);
		$myString .= $this->returneHidden('sna', $this->show_not_allow);
		$myString .= $this->returneHidden('aut', $this->auto_submit);
		$myString .= $this->returneHidden('frm', $this->form_container);
		$myString .= $this->returneHidden('tar', $this->target_url);
		$myString .= $this->returneHidden('inp', $this->show_input);
		$myString .= $this->returneHidden('fmt', $this->date_format);
		$myString .= $this->returneHidden('dis', implode(",", $this->dsb_days));
		$myString .= $this->returneHidden('pr1', $this->date_pair1);
		$myString .= $this->returneHidden('pr2', $this->date_pair2);
		$myString .= $this->returneHidden('prv', $this->date_pair_value);
		$myString .= $this->returneHidden('pth', $this->path);

		$myString .= $this->returneHidden('spd', $this->check_json_encode($this->sp_dates));
		$myString .= $this->returneHidden('spt', $this->sp_type);
	...
	function getMonthNames(){
		return array("Januar", "Februar", "M&auml;rz", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember");
	}
	...
	function returnWriteDateContainer()
	{
		if($this->day && $this->month && $this->year)
			$dd = date($this->date_format, mktime(0,0,0,$this->month,$this->day,$this->year));
		else $dd = "Select Date";

		return("<span id=\"divCalendar_".$this->objname."_lbl\" class=\"date-tccontainer\"".($this->rtl ? " dir=\"rtl\"" : "").">$dd</span>");
	}
